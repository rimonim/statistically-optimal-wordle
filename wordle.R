...
common_seconds <- data.frame(green = c(00, 20, 00, 00, 00, 20, 10, 20, 250, 450, 00, 00, 40),
                             yellow = c(00, 20, 40, 30, 50, 240, 150, 250, 250, 450, 3450, 240, 340),
                             optimal_guess = c("slimy", "bludy", "shunt", "lysin", "silen", "cumin", "plier", "wynds", "gusli", "muils", "splat", "punch", "spiny"))
...
play <- function(answer){
  ...
  for (try in 1:5) {
    ...
    green <- as.integer(paste(c(which(unlist(strsplit("roate", "")) == answer), 0), collapse = ""))
    yellow <- as.integer(paste(c(which(unlist(strsplit("roate", "")) %in% answer), 0), collapse = ""))
    if (try == 1 & length(common_seconds$optimal_guess[common_seconds$green == green & common_seconds$yellow == yellow]) >= 1) {
